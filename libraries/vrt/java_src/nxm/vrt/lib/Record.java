...
public class Record implements HasFields {
  ...
  /** <b>Not for General Use:</b> Creates a new instance sharing the buffers of the one specified.
  ...
  /** Creates a copy of this record. Note that the version in {@link Record} may return a
   *  {@link Record} and not the same sub-class as was given.
   *  @return A copy of this record that does not share data with the original.
   */
  public Record copy () {
  ...
  /** Gets the length in octets.
   *  @return The record length in octets.
   */
  public int getByteLength () {
    return buf.length;
  }

  /** Sets the length in octets.
   *  @param len The record length in octets.
   */
  protected void setByteLength (int len) {
    setByteLength(len, -1);
  }

  /** Sets the total length in octets with any additional octets added/removed from the
   *  given point (-1=end). This is used to handle insertion/removal from the mid-point *or*
   *  end of a record.
   *  @param len The new length of the record in octets.
   *  @param off The offset into the current record from where any octets should be added/removed.
   *             (-1 = add/remove from end).
   */
