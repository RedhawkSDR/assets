...
if [ "$1" = "rpm" ]; then
    ...
    if [ -e rh.RBDSDecoder.spec ]; then
        mydir=`dirname $0`
        tmpdir=`mktemp -d`
        cp -r ${mydir} ${tmpdir}/rh.RBDSDecoder-2.0.3
        tar czf ${tmpdir}/rh.RBDSDecoder-2.0.3.tar.gz --exclude=".svn" -C ${tmpdir} rh.RBDSDecoder-2.0.3
        rpmbuild -ta ${tmpdir}/rh.RBDSDecoder-2.0.3.tar.gz
        rm -rf $tmpdir
    else
