...
class SddsHeader:
    ...
    def __init__(self, FSN,
                 SF = 1, SoS = 1, PP = 0, OF = 0, SS = 0, DM = [0, 0, 1], BPS = [1, 0, 0, 0, 0],
                 MSV = 0, TTV = 1, SSV = 1,
                 MSD = 0, TT = 0, TTE = 0,
...
class SddsCharPacket(SddsPacket):
    ...
    def decode(self):
        return struct.unpack('>HHHHQLLQ24B1024B', self.encodedPacket)
...
class SddsShortPacket(SddsPacket):
    ...
    def decode(self):
        return struct.unpack('>HHHHQLLQ24B512H', self.encodedPacket)
...
class SddsFloatPacket(SddsPacket):
    ...
    def decode(self):
        return struct.unpack('>HHHHQLLQ24B256f', self.encodedPacket)
