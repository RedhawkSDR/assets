...
if [ "$1" = "rpm" ]; then
    ...
    if [ -e rh.HardLimit.spec ]; then
        mydir=`dirname $0`
        tmpdir=`mktemp -d`
        cp -r ${mydir} ${tmpdir}/rh.HardLimit-2.0.2
        tar czf ${tmpdir}/rh.HardLimit-2.0.2.tar.gz --exclude=".svn" -C ${tmpdir} rh.HardLimit-2.0.2
        rpmbuild -ta ${tmpdir}/rh.HardLimit-2.0.2.tar.gz
        rm -rf $tmpdir
    else
