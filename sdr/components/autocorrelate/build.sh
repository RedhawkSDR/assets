...
if [ "$1" = "rpm" ]; then
    ...
    if [ -e rh.autocorrelate.spec ]; then
        mydir=`dirname $0`
        tmpdir=`mktemp -d`
        cp -r ${mydir} ${tmpdir}/rh.autocorrelate-2.0.2
        tar czf ${tmpdir}/rh.autocorrelate-2.0.2.tar.gz --exclude=".svn" -C ${tmpdir} rh.autocorrelate-2.0.2
        rpmbuild -ta ${tmpdir}/rh.autocorrelate-2.0.2.tar.gz
        rm -rf $tmpdir
    else
