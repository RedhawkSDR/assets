...
if [ "$1" = "rpm" ]; then
    ...
    if [ -e rh.TuneFilterDecimate.spec ]; then
        mydir=`dirname $0`
        tmpdir=`mktemp -d`
        cp -r ${mydir} ${tmpdir}/rh.TuneFilterDecimate-2.0.3
        tar czf ${tmpdir}/rh.TuneFilterDecimate-2.0.3.tar.gz --exclude=".svn" -C ${tmpdir} rh.TuneFilterDecimate-2.0.3
        rpmbuild -ta ${tmpdir}/rh.TuneFilterDecimate-2.0.3.tar.gz
        rm -rf $tmpdir
    else
