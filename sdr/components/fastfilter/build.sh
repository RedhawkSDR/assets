...
if [ "$1" = "rpm" ]; then
    ...
    if [ -e rh.fastfilter.spec ]; then
        mydir=`dirname $0`
        tmpdir=`mktemp -d`
        cp -r ${mydir} ${tmpdir}/rh.fastfilter-2.1.0
        tar czf ${tmpdir}/rh.fastfilter-2.1.0.tar.gz --exclude=".svn" -C ${tmpdir} rh.fastfilter-2.1.0
        rpmbuild -ta ${tmpdir}/rh.fastfilter-2.1.0.tar.gz
        rm -rf $tmpdir
    else
