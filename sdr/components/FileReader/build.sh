...
if [ "$1" = "rpm" ]; then
    ...
    if [ -e rh.FileReader.spec ]; then
        mydir=`dirname $0`
        tmpdir=`mktemp -d`
        cp -r ${mydir} ${tmpdir}/rh.FileReader-5.0.1
        tar czf ${tmpdir}/rh.FileReader-5.0.1.tar.gz --exclude=".svn" --exclude=".git" -C ${tmpdir} rh.FileReader-5.0.1
        rpmbuild -ta ${tmpdir}/rh.FileReader-5.0.1.tar.gz
        rm -rf $tmpdir
    else
