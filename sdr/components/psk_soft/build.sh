...
if [ "$1" = "rpm" ]; then
    ...
    if [ -e rh.psk_soft.spec ]; then
        mydir=`dirname $0`
        tmpdir=`mktemp -d`
        cp -r ${mydir} ${tmpdir}/rh.psk_soft-2.0.3
        tar czf ${tmpdir}/rh.psk_soft-2.0.3.tar.gz --exclude=".svn" -C ${tmpdir} rh.psk_soft-2.0.3
        rpmbuild -ta ${tmpdir}/rh.psk_soft-2.0.3.tar.gz
        rm -rf $tmpdir
    else
