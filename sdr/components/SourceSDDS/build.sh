...
if [ "$1" = "rpm" ]; then
    ...
    if [ -e rh.SourceSDDS.spec ]; then
        mydir=`dirname $0`
        tmpdir=`mktemp -d`
        cp -r ${mydir} ${tmpdir}/rh.SourceSDDS-2.0.3
        tar czf ${tmpdir}/rh.SourceSDDS-2.0.3.tar.gz --exclude=".svn" --exclude=".git" -C ${tmpdir} rh.SourceSDDS-2.0.3
        rpmbuild -ta ${tmpdir}/rh.SourceSDDS-2.0.3.tar.gz
        rm -rf $tmpdir
    else
