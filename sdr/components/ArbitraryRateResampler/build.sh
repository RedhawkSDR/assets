...
if [ "$1" = "rpm" ]; then
    ...
    if [ -e rh.ArbitraryRateResampler.spec ]; then
        mydir=`dirname $0`
        tmpdir=`mktemp -d`
        cp -r ${mydir} ${tmpdir}/rh.ArbitraryRateResampler-2.0.2
        tar czf ${tmpdir}/rh.ArbitraryRateResampler-2.0.2.tar.gz --exclude=".svn" -C ${tmpdir} rh.ArbitraryRateResampler-2.0.2
        rpmbuild -ta ${tmpdir}/rh.ArbitraryRateResampler-2.0.2.tar.gz
        rm -rf $tmpdir
    else
