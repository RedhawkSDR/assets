...
if [ "$1" = "rpm" ]; then
    ...
    if [ -e rh.SourceVITA49.spec ]; then
        mydir=`dirname $0`
        tmpdir=`mktemp -d`
        cp -r ${mydir} ${tmpdir}/rh.SourceVITA49-3.1.2
        tar czf ${tmpdir}/rh.SourceVITA49-3.1.2.tar.gz --exclude=".svn" -C ${tmpdir} rh.SourceVITA49-3.1.2
        rpmbuild -ta ${tmpdir}/rh.SourceVITA49-3.1.2.tar.gz
        rm -rf $tmpdir
    else
