...
if [ "$1" = "rpm" ]; then
    ...
    if [ -e rh.RedhawkDevUtils.spec ]; then
        mydir=`dirname $0`
        tmpdir=`mktemp -d`
        cp -r ${mydir} ${tmpdir}/rh.RedhawkDevUtils-5.0.1
        tar czf ${tmpdir}/rh.RedhawkDevUtils-5.0.1.tar.gz --exclude=".svn" -C ${tmpdir} rh.RedhawkDevUtils-5.0.1
        rpmbuild -ta ${tmpdir}/rh.RedhawkDevUtils-5.0.1.tar.gz
        rm -rf $tmpdir
    else
