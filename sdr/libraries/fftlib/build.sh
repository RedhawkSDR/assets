...
if [ "$1" = "rpm" ]; then
    ...
    if [ -e rh.fftlib.spec ]; then
        mydir=`dirname $0`
        tmpdir=`mktemp -d`
        cp -r ${mydir} ${tmpdir}/rh.fftlib-2.1.2
        tar czf ${tmpdir}/rh.fftlib-2.1.2.tar.gz --exclude=".svn" -C ${tmpdir} rh.fftlib-2.1.2
        rpmbuild -ta ${tmpdir}/rh.fftlib-2.1.2.tar.gz
        rm -rf $tmpdir
    else
