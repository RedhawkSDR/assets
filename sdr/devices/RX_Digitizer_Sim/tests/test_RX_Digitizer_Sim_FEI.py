...
# Your custom device
dut_config['RX_Digitizer_Sim'] = {
    # parent_dir/namespace/name are only used when spd = None
    'spd'         : os.path.join(project_dir, 'RX_Digitizer_Sim.spd.xml'),       # path to SPD file for asset.
    'parent_dir'  : None,       # path to parent directory
    'namespace'   : None,       # leave as None if asset is not namespaced
    'name'        : 'RX_Digitizer_Sim', # exact project name, without namespace
    'impl_id'     : 'python',       # None for all (or only), 'cpp', 'python', 'java', etc.
