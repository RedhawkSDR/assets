...
class FrontendTunerTests(ParameterizedTestCase,unittest.TestCase):
    ...
    def hasChannelizer(self):
        rcv_id = DEVICE_INFO['receiver_identifier']
        if  not DEVICE_INFO['capabilities'][rcv_id][0].has_key("CHANNELIZER") and \
                not DEVICE_INFO['capabilities'][rcv_id][0].has_key("RX_DIGITIZER_CHANNELIZER"):
            return False
        if DEVICE_INFO['capabilities'][rcv_id][0].has_key("CHANNELIZER"):
            self.mychannelizer=DEVICE_INFO['capabilities'][rcv_id][0]['CHANNELIZER']
            self.mychannelizer_type='CHANNELIZER'
        if DEVICE_INFO['capabilities'][rcv_id][0].has_key("RX_DIGITIZER_CHANNELIZER"):
            self.mychannelizer=DEVICE_INFO['capabilities'][rcv_id][0]['RX_DIGITIZER_CHANNELIZER']
    ...
    def attachableOutput(self, tids ):
        data_proto=None
        psd_proto=None
        if  DEVICE_INFO['configure'].has_key('tuner_output'):
            for x in DEVICE_INFO['configure']['tuner_output']:
                if  x['tuner_output::tuner_number'] in tids and  \
                    x['tuner_output::enabled'] :
                    data_proto=x['tuner_output::protocol']
                    break

        if  DEVICE_INFO['configure'].has_key('block_psd_output'):
            for x in DEVICE_INFO['configure']['block_psd_output']:
                for tid in tids:
                    if ( tid >= x['block_psd_output::fft_channel_start'] and  \
                         tid <= x['block_psd_output::fft_channel_stop']) and  \
                        x['block_psd_output::enabled']:
                        psd_proto=x['block_psd_output::protocol']
                        break
    ...
    @classmethod
    def getToBasicState(self, spd_file, execparams={}, configure={}, initialize=True, receiver_identifier=None ):
        ...
        try:
            ...
            comp = sb.launch(spd_file,execparams=execparams,configure=configure,initialize=initialize,impl=IMPL_ID)
        except:
            # deprecated, use in 1.8.x versions
            comp = sb.Component(spd_file,execparams=execparams,configure=configure,initialize=initialize,impl=IMPL_ID)

        # handle case for parent/child devices
        dut_ref =  comp.ref._narrow(CF.AggregateDevice)
        if not dut_ref:
            dut_ref = comp.ref._narrow(CF.Device)
            dut=comp
        else:
            rcv_id=None
            if receiver_identifier:
                rcv_id=receiver_identifier
            for cdev in comp.ref.devices:
                d=createSandboxDevice(cdev)
                if not rcv_id:
                    dut_ref=cdev
                    dut=d
                    break;
        ...
        if not dut_ref:
            self.fail( "Unable to find Frontend Device to test ..")
        
        self.comp =comp             # component that we launched and used for cleanup
        self.dut = dut              # sandbox wrapped device that we are testings against
        self.dut_ref=dut_ref        # CF device object that we are testing against
    ...
    def setUpClass(self):
        ...
        self.getToBasicState( spd_file=DEVICE_INFO['spd'],
                              ...
                              receiver_identifier=DEVICE_INFO['receiver_identifier'])
    ...
    def cfg_channel_output(self, out_cfg_definitions, ch_list, enable_disable=True):
        try:
            ...
            for ch_id in ch_list:
                try:
                    out_cfg_definitions[ch_id]['tuner_output::enabled'] = enable_disable
                except:
                    pass
        except:
            pass
    ...
    def process_attributes(self, test_func):
        """
        Handle method decorates that will enable, disable tuner channel output
        """
        ...
        if tm:
            ...
            out_cfg=self.dut_properties['tuner_output']
    ...
    def setUp(self):
        ...
        # get property settings for the device under test
        ...
        try:
            ipaddr=self.dut_properties['msdd']['msdd::ip_address']
    ...
    def _testFRONTEND_3_4_DataFlow(self, port_num):
        ...
        tuner_control = self.dut.getPort('DigitalTuner_in')
        count = 0
        for port in sorted([ x for x in self.dut.getPortSet() if x.direction.upper() == 'USES' ], key=lambda port: port.name):
            comp_port_type = port.repid.split(':')[1].split('/')[-1]
    ...
    def testFRONTEND_2_5(self):
        ...
        '''
        rcv_id = DEVICE_INFO['receiver_identifier']
        if "RX_DIGITIZER_CHANNELIZER" in DEVICE_INFO['capabilities'][rcv_id][0]:
    ...
    def testFRONTEND_2_5_1(self):
        ...
        if "RX_DIGITIZER_CHANNELIZER" in DEVICE_INFO['capabilities'][rcv_id][0]:
            ...
            t2=None
            t2Alloc=None
            if "DDC" in DEVICE_INFO['capabilities'][rcv_id][0] and \
                DEVICE_INFO['capabilities'][rcv_id][0]['DDC'] is not None:
    ...
    def testFRONTEND_2_5_2(self):
        ...
        if "RX_DIGITIZER_CHANNELIZER" in DEVICE_INFO['capabilities'][rcv_id][0]:
            ...
            if "DDC" in DEVICE_INFO['capabilities'][rcv_id][0] and \
                DEVICE_INFO['capabilities'][rcv_id][0]['DDC'] is not None:
                numDDCs = DEVICE_INFO['capabilities'][rcv_id][0]["DDC"]["NUMDDCs"]
    ...
    def testAllDDCsAllocation(self):
        ...
        ddcAllocs = []
        rcv_id = DEVICE_INFO['receiver_identifier']
        for idx in range(len(DEVICE_INFO['capabilities'][rcv_id])):
            if DEVICE_INFO['capabilities'][rcv_id][idx].has_key('DDC') and \
               DEVICE_INFO['capabilities'][rcv_id][idx]['DDC'] is not None:
                for tuner_num in range(DEVICE_INFO['capabilities'][rcv_id][idx]['DDC']['NUMDDCs']):
                    dev_state=self.dut_ref._get_usageState()
                    if dev_state != CF.Device.BUSY:
    ...
    def testChannelizerAllocationOrder1(self):
        rcv_id = DEVICE_INFO['receiver_identifier']
        if 'CHANNELIZER' not in DEVICE_INFO['capabilities'][rcv_id][0]:
            return
        xdelta = 1.0/(DEVICE_INFO['capabilities'][rcv_id][0]['CHANNELIZER']['SR'][1])
    ...
    def testChannelizerAllocationSR(self):
        ...
        xdelta = 2.0/(DEVICE_INFO['capabilities'][rcv_id][0]['CHANNELIZER']['SR'][1])
        sri = BULKIO.StreamSRI(hversion=1, xstart=0.0, xdelta=xdelta, xunits=1, subsize=0, ystart=0.0, ydelta=0.0, yunits=0, mode=0, streamID='TestStreamID', blocking=False, keywords=[])
        port = self.dut_ref.getPort("dataShort_in")
        try:
            port.pushSRI(sri)
...
def getMaxCenterFreq(idx=None,ttype="RX_DIGITIZER"):
    ...
    rcv_id = DEVICE_INFO['receiver_identifier']
    if DEBUG_LEVEL >= 4:
        pp(DEVICE_INFO['capabilities'][rcv_id])

    max_cf = 0
    if idx == None:
        for chan in  DEVICE_INFO['capabilities'][rcv_id]:
            if chan.has_key(ttype):
                max_cf = max(max_cf,chan[ttype]['CF'][-1])
    else:
        try:
            max_cf = DEVICE_INFO['capabilities'][rcv_id][idx][ttype]['CF'][-1]
        except IndexError:
            # bad idx value, just return idx 0
            max_cf = DEVICE_INFO['capabilities'][rcv_id][0][ttype]['CF'][-1]
...
def getMaxSampleRate(idx=None, cap=True,ttype="RX_DIGITIZER"):
    ...
    if idx == None:
        ...
        try:
            max_sr = DEVICE_INFO['capabilities'][rcv_id][idx][ttype]['SR'][-1]
        except IndexError:
            # bad idx value, just return idx 0
            max_sr = DEVICE_INFO['capabilities'][rcv_id][0][ttype]['SR'][-1]
...
def getMaxBandwidth(idx=None,ttype="RX_DIGITIZER"):
    ...
    if idx == None:
        ...
        try:
            max_bw = DEVICE_INFO['capabilities'][rcv_id][idx][ttype]['BW'][-1]
        except IndexError:
            # bad idx value, just return idx 0
            max_bw = DEVICE_INFO['capabilities'][rcv_id][0][ttype]['BW'][-1]
    return max_bw 
...
def getMinCenterFreq(idx=None,ttype="RX_DIGITIZER"):
    ...
    rcv_id = DEVICE_INFO['receiver_identifier']
    min_cf = DEVICE_INFO['capabilities'][rcv_id][0][ttype]['CF'][0]
    if idx == None:
        for chan in  DEVICE_INFO['capabilities'][rcv_id]:
            if chan.has_key(ttype):
        ...
        try:
            min_cf = DEVICE_INFO['capabilities'][rcv_id][idx][ttype]['CF'][0]
        except IndexError:
            # bad idx value, just return idx 0
            min_cf = DEVICE_INFO['capabilities'][rcv_id][0][ttype]['CF'][0]
...
def getMinSampleRate(idx=None,ttype="RX_DIGITIZER"):
    # If idx==None, iterate through all and find min
    rcv_id = DEVICE_INFO['receiver_identifier']
    min_sr = DEVICE_INFO['capabilities'][rcv_id][0][ttype]['SR'][0]
    if idx == None:
        for chan in  DEVICE_INFO['capabilities'][rcv_id]:
            if chan.has_key(ttype):
        ...
        try:
            min_sr = DEVICE_INFO['capabilities'][rcv_id][idx][ttype]['SR'][0]
        except IndexError:
            # bad idx value, just return idx 0
            min_sr = DEVICE_INFO['capabilities'][rcv_id][0][ttype]['SR'][0]
...
def getMinBandwidth(idx=None,ttype="RX_DIGITIZER"):
    # If idx==None, iterate through all and find min
    rcv_id = DEVICE_INFO['receiver_identifier']
    min_bw = DEVICE_INFO['capabilities'][rcv_id][0][ttype]['BW'][0]
    if idx == None:
        for chan in  DEVICE_INFO['capabilities'][rcv_id]:
            if chan.has_key(ttype):
        ...
        try:
            min_bw = DEVICE_INFO['capabilities'][rcv_id][idx][ttype]['BW'][0]
        except IndexError:
            # bad idx value, just return idx 0
            min_bw = DEVICE_INFO['capabilities'][rcv_id][0][ttype]['BW'][0]
    return min_bw 
...
def getMaxGain(idx=None,ttype="RX_DIGITIZER"):
    # If idx==None, iterate through all and find min
    rcv_id = DEVICE_INFO['receiver_identifier']
    max_gain = DEVICE_INFO['capabilities'][rcv_id][0][ttype]['GAIN'][-1]
    if idx == None:
        for chan in  DEVICE_INFO['capabilities'][rcv_id]:
            if chan.has_key(ttype):
        ...
        try:
            max_gain = DEVICE_INFO['capabilities'][rcv_id][idx][ttype]['GAIN'][-1]
        except IndexError:
            # bad idx value, just return idx 0
            max_gain = DEVICE_INFO['capabilities'][rcv_id][0][ttype]['GAIN'][-1]
...
def getMinGain(idx=None,ttype="RX_DIGITIZER"):
    # If idx==None, iterate through all and find min
    rcv_id = DEVICE_INFO['receiver_identifier']
    min_gain = DEVICE_INFO['capabilities'][rcv_id][0][ttype]['GAIN'][0]
    if idx == None:
        for chan in  DEVICE_INFO['capabilities'][rcv_id]:
            if chan.has_key(ttype):
        ...
        try:
            min_gain = DEVICE_INFO['capabilities'][rcv_id][idx][ttype]['GAIN'][0]
        except IndexError:
            # bad idx value, just return idx 0
            min_gain = DEVICE_INFO['capabilities'][rcv_id][0][ttype]['GAIN'][0]
...
def generateChannelizerRequest(idx=0,sr=0,bw=0):
    ...
    rcv_id = DEVICE_INFO['receiver_identifier']
    if not sr:
        value['SR'] = DEVICE_INFO['capabilities'][rcv_id][idx]['CHANNELIZER']['SR'][1] #Allocated at Max SR
    else:
        value['SR'] = sr
    if not bw:
        value['BW'] = DEVICE_INFO['capabilities'][rcv_id][idx]['CHANNELIZER']['BW'][1] #Allocated at Max BW
    else:
        value['BW'] = bw    
    
    value['CF'] = getValueInRange(DEVICE_INFO['capabilities'][rcv_id][idx]['CHANNELIZER']['CF'])
...
def generateRXDigitizerChannelizerRequest(idx=0):
    ...
    value['BW'] = 0 
    rcv_id = DEVICE_INFO['receiver_identifier']    
    value['CF'] = getValueInRange(DEVICE_INFO['capabilities'][rcv_id][idx]['RX_DIGITIZER_CHANNELIZER']['CF'])
...
def generateDDCRequest(idx = 0,cf=0):
    ...
    bw=None
    rcv_id = DEVICE_INFO['receiver_identifier']
    if DEVICE_INFO['capabilities'][rcv_id][idx]['DDC'].has_key("RESTRICT"):
        sr, bw = getRestrictedValues(DEVICE_INFO['capabilities'][rcv_id][idx]['DDC']['SR'],
                                    DEVICE_INFO['capabilities'][rcv_id][idx]['DDC']['BW'])
        value['SR']=sr
        value['BW']=bw     

    if  not sr:
        sr_subrange = DEVICE_INFO['capabilities'][rcv_id][idx]['DDC']['SR']
        if 'sr_limit' in DEVICE_INFO and DEVICE_INFO['sr_limit'] > 0:
            sr_subrange = getSubranges([0,DEVICE_INFO['sr_limit']], DEVICE_INFO['capabilities'][rcv_id][idx]['DDC']['SR'])
        value['SR'] = getValueInRange(sr_subrange)

    if not bw:
        # Usable BW is typically equal to SR if complex samples, otherwise half of SR
        BW_MULT = 1.0 if DEVICE_INFO['capabilities'][rcv_id][idx]['DDC']['COMPLEX'] else 0.5
        value['BW'] = 0.8*value['SR']*BW_MULT # Try 80% of SR
        if isValueInRange(value['BW'], DEVICE_INFO['capabilities'][rcv_id][idx]['DDC']['BW']):
        ...
        bw_subrange = getSubranges(tolerance_range, DEVICE_INFO['capabilities'][rcv_id][idx]['DDC']['BW'])
        ...
        # last resort
        value['BW'] = getValueInRange(DEVICE_INFO['capabilities'][rcv_id][idx]['DDC']['BW'])
...
def defaultGenerateTunerRequest(idx=0):
    ...
    if get_debug_level() > 4:
        print "*********** idx ", idx
        print "*********** dut DEVICE_INFO['capabilities'][rcv_id][idx]['RX_DIGITIZER']", len(DEVICE_INFO['capabilities'][rcv_id])
    pp(DEVICE_INFO['capabilities'][rcv_id])
    value['CF'] = getValueInRange(DEVICE_INFO['capabilities'][rcv_id][idx]['RX_DIGITIZER']['CF'])
    sr_subrange = DEVICE_INFO['capabilities'][rcv_id][idx]['RX_DIGITIZER']['SR']
    if 'sr_limit' in DEVICE_INFO and DEVICE_INFO['sr_limit'] > 0:
        sr_subrange = getSubranges([0,DEVICE_INFO['sr_limit']], DEVICE_INFO['capabilities'][rcv_id][idx]['RX_DIGITIZER']['SR'])
    value['SR'] = getValueInRange(sr_subrange)
    # Usable BW is typically equal to SR if complex samples, otherwise half of SR
    BW_MULT = 1.0 if DEVICE_INFO['capabilities'][rcv_id][idx]['RX_DIGITIZER']['COMPLEX'] else 0.5
    value['BW'] = 0.8*value['SR']*BW_MULT # Try 80% of SR
    if isValueInRange(value['BW'], DEVICE_INFO['capabilities'][rcv_id][idx]['RX_DIGITIZER']['BW']):
        # success! all done, return value
        return value
    ...
    bw_max = value['SR']*(1.0+(value['SR_TOLERANCE']/100.0))*BW_MULT
    tolerance_range = [bw_min,bw_max]
    bw_subrange = getSubranges(tolerance_range, DEVICE_INFO['capabilities'][rcv_id][idx]['RX_DIGITIZER']['BW'])
    if len(bw_subrange) > 0:
        # success! get BW value and return
        value['BW'] = getValueInRange(bw_subrange)
        return value
    ...
    # last resort
    value['BW'] = getValueInRange(DEVICE_INFO['capabilities'][rcv_id][idx]['RX_DIGITIZER']['BW'])
    return value
