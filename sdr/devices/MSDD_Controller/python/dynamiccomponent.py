...
import threading
'''
DynamicComponent was taken from the FEI 3.0 beta release to allow for easier migration.
...
class DynamicComponent:
    ...
    def addChild(self, instance, moduleName=None, instanceName=None, exec_params=None):
        device_object = None
        try:
            ...
            with ossie.device.envState():
                ...
                parameters = []
                if device_name not in self._dynamicComponentCount:
                ...
                if exec_params:
                    execparams.update(exec_params)

                _mod_name = device_name
                if moduleName:
                ...
                try:
                    ...
                    traceback.print_exc()
                    kclass = getattr(mod, device_name)
