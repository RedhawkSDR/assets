...
if [ "$1" = "rpm" ]; then
    ...
    if [ -e rh.RTL2832U.spec ]; then
        mydir=`dirname $0`
        tmpdir=`mktemp -d`
        cp -r ${mydir} ${tmpdir}/rh.RTL2832U-2.1.2
        tar czf ${tmpdir}/rh.RTL2832U-2.1.2.tar.gz --exclude=".svn" --exclude=".git" -C ${tmpdir} rh.RTL2832U-2.1.2
        rpmbuild -ta ${tmpdir}/rh.RTL2832U-2.1.2.tar.gz
        rm -rf $tmpdir
    else
