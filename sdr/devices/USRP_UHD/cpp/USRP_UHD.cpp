...
void USRP_UHD_i::constructor()
{
    ...
    ***********************************************************************************/
    ...
    if (dev_addrs.size() > 1) {
        ...
        errstr << "Ambiguous USRP_UHD. Found "<<dev_addrs.size()<<" instead of just 1. Try setting the ip_address property";
        ...
        if (this->ip_address.empty()) {
            errstr = "No USRP_UHD found";
        } else {
            std::stringstream serrstr;
            serrstr << "The specified IP address for this USRP_UHD ("<<this->ip_address<<") is not responsive";
            errstr = serrstr.str();
        }
    ...
    setPropertyQueryImpl(frontend_tuner_status, this, &USRP_UHD_i::get_fts);
}

std::vector<frontend_tuner_status_struct_struct> USRP_UHD_i::get_fts()
{
...
void USRP_UHD_i::frontendTunerStatusChanged(const std::vector<frontend_tuner_status_struct_struct>* oldValue, const std::vector<frontend_tuner_status_struct_struct>* newValue)
{
}

bool USRP_UHD_i::_synchronizeClock(const std::string source)
{
    ...
    if (source == "GPSDO") {
        ...
        // Synchronize the USRP_UHD clock to GPS time by waiting for a PPS edge 
        ...
        // Synchronize the USRP_UHD clock as closely as possible with the host by
...
/***********************************************************************************************
    ...
    Messages:
        ...
        void USRP_UHD_i::my_message_callback(const std::string& id, const my_msg_struct &msg){
        }
        
        Register the message callback onto the input port with the following form:
        this->msg_input->registerMessage("my_msg", this, &USRP_UHD_i::my_message_callback);
    ...
    Properties:
        ...
        addPropertyListener(<property>, this, &USRP_UHD_i::<callback method>)
        in the constructor.
        ...
        {
            addPropertyListener(scaleValue, this, &USRP_UHD_i::scaleChanged);
            addPropertyListener(status, this, &USRP_UHD_i::statusChanged);
        }

        void USRP_UHD_i::scaleChanged(float oldValue, float newValue)
        {
            RH_DEBUG(this->_baseLog, "scaleValue changed from" << oldValue << " to " << newValue);
        }
            
        void USRP_UHD_i::statusChanged(const status_struct& oldValue, const status_struct& newValue)
        {
    ...
    Allocation:
        ...
        bool USRP_UHD_i::my_alloc_fn(const std::string &value)
        {
            // perform logic
            return true; // successful allocation
        }
        void USRP_UHD_i::my_dealloc_fn(const std::string &value)
        {
            // perform logic
        }
        ...
        this->setAllocationImpl(my_alloc, this, &USRP_UHD_i::my_alloc_fn, &USRP_UHD_i::my_dealloc_fn);
...
CF::Device::Allocations* USRP_UHD_i::allocate(const CF::Properties& capacities)
...
void USRP_UHD_i::deallocate (const char* alloc_id)
...
bool USRP_UHD_i::deviceSetTuningScan(const frontend::frontend_tuner_allocation_struct &request, const frontend::frontend_scanner_allocation_struct &scan_request, frontend_tuner_status_struct_struct &fts, size_t tuner_id){
    /************************************************************
     *  not used. All allocations delegated to children
    ************************************************************/
    return false;
...
void USRP_UHD_i::configureTuner(const std::string& allocation_id, const CF::Properties& tunerSettings){
...
CF::Properties* USRP_UHD_i::getTunerSettings(const std::string& allocation_id){
    long idx = getTunerMapping(allocation_id);
    if (idx < 0) {
        if (_delegatedAllocations.find(allocation_id) != _delegatedAllocations.end()) {
            FRONTEND::DigitalTuner_ptr port = FRONTEND::DigitalTuner::_narrow(_delegatedAllocations[allocation_id][0].control_ports[0].port_ref);
...
frontend::ScanStatus USRP_UHD_i::getScanStatus(const std::string& allocation_id) {
    long idx = getTunerMapping(allocation_id);
    if (idx < 0) {
        if (_delegatedAllocations.find(allocation_id) != _delegatedAllocations.end()) {
            ...
            if (port) {
                    ...
                    case FRONTEND::ScanningTuner::SPAN_SCAN:
                        {
                            ...
                            frontend::ScanStatus retval(_strat);
                ...
                frontend::ManualStrategy* _strat = new frontend::ManualStrategy(tmpVal->strategy.scan_definition.center_frequency());
                switch(tmpVal->strategy.control_mode) {
                    case frontend::TIME_BASED:
                        _strat->control_mode = frontend::TIME_BASED;
                        break;
...
void USRP_UHD_i::setScanStartTime(const std::string& allocation_id, const BULKIO::PrecisionUTCTime& start_time) {
    long idx = getTunerMapping(allocation_id);
    if (idx < 0) {
        if (_delegatedAllocations.find(allocation_id) != _delegatedAllocations.end()) {
            FRONTEND::DigitalScanningTuner_ptr port = FRONTEND::DigitalScanningTuner::_narrow(_delegatedAllocations[allocation_id][0].control_ports[0].port_ref);
...
void USRP_UHD_i::setScanStrategy(const std::string& allocation_id, const frontend::ScanStrategy* scan_strategy) {
    long idx = getTunerMapping(allocation_id);
    if (idx < 0) {
        if (_delegatedAllocations.find(allocation_id) != _delegatedAllocations.end()) {
            FRONTEND::DigitalScanningTuner_ptr port = FRONTEND::DigitalScanningTuner::_narrow(_delegatedAllocations[allocation_id][0].control_ports[0].port_ref);
            if (port) {
                ...
                return port->setScanStrategy(allocation_id.c_str(), tmp_strat);
            }
        }
    }
}
