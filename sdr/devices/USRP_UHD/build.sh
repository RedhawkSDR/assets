...
if [ "$1" = "rpm" ]; then
    ...
    if [ -e rh.USRP_UHD.spec ]; then
        mydir=`dirname $0`
        tmpdir=`mktemp -d`
        cp -r ${mydir} ${tmpdir}/rh.USRP_UHD-6.1.1
        tar czf ${tmpdir}/rh.USRP_UHD-6.1.1.tar.gz --exclude=".svn" --exclude=".git" -C ${tmpdir} rh.USRP_UHD-6.1.1
        rpmbuild -ta ${tmpdir}/rh.USRP_UHD-6.1.1.tar.gz
        rm -rf $tmpdir
    else
