...
if [ "$1" = "rpm" ]; then
    ...
    if [ -e rh.USRP.spec ]; then
        mydir=`dirname $0`
        tmpdir=`mktemp -d`
        cp -r ${mydir} ${tmpdir}/rh.USRP-7.0.0
        tar czf ${tmpdir}/rh.USRP-7.0.0.tar.gz --exclude=".svn" --exclude=".git" -C ${tmpdir} rh.USRP-7.0.0
        rpmbuild -ta ${tmpdir}/rh.USRP-7.0.0.tar.gz
        rm -rf $tmpdir
    else
