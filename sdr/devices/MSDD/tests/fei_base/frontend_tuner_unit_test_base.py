...
class FrontendTunerTests(ParameterizedTestCase,unittest.TestCase):
    ...
    def hasChannelizer(self):
        ...
        if  "CHANNELIZER" not in DEVICE_INFO['capabilities'][rcv_id][0] and \
                "RX_DIGITIZER_CHANNELIZER" not in DEVICE_INFO['capabilities'][rcv_id][0]:
            return False
        if "CHANNELIZER" in DEVICE_INFO['capabilities'][rcv_id][0]:
    ...
    def attachableOutput(self, tids ):
        data_proto=None
        psd_proto=None
        if  'tuner_output' in DEVICE_INFO['configure']:
        ...
        if  'block_psd_output' in DEVICE_INFO['configure']:
    ...
    def cfg_channel_output(self, out_cfg_definitions, ch_list, enable_disable=True):
        try:
            if len(ch_list)== 0:
                ch_list=list(range(len(out_cfg)))
    ...
    def testFRONTEND_3_1_3(self): 
        ...
        try:
            if not self.check(self.dut_ref.allocateCapacity(over_tAlloc), False, 'Over-allocate RX_DIGITIZER check') and DEBUG_LEVEL >= 4:
                ...
                pp(over_t)
                pp(over_tAlloc)
        except CF.Device.InvalidState as e :
    ...
    def testFRONTEND_2_5_2(self):
        ...
        if "RX_DIGITIZER_CHANNELIZER" in DEVICE_INFO['capabilities'][rcv_id][0]:
            ...
            #Deallocate all DDCs
            error = False
            for tuner_num in range(0,len(allocations)):
                try:
    ...
    def testAllDDCsAllocation(self):
        ...
        for idx in range(len(DEVICE_INFO['capabilities'][rcv_id])):
            if 'DDC' in DEVICE_INFO['capabilities'][rcv_id][idx] and \
...
def generateDDCRequest(idx = 0,cf=0):
    ...
    bw=None
    rcv_id = DEVICE_INFO['receiver_identifier']
    if "RESTRICT" in DEVICE_INFO['capabilities'][rcv_id][idx]['DDC']:
