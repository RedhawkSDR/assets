...
#warning "Since Dec 2013 (when support for UDP/Unicast and TCP was added to \
          NetUtilities and the UDP/Multicast code was moved there with it) \
          the multicast.h header is no longer supported."
...
#define MCAST_ADDRESS_MASK 0x000000E0
 ...
 *  @param is_receiver  Is the socket a receiver? (will not bind socket if not).
 *  @return The multicast tag.
 */
static multicast_t multicast_open_(const char* iface, const char* group,
    int port, bool is_receiver) {
  size_t ii;
  int one = 1;
  ...
  in_addr ip_addr;
  if (strcmp(group,"") != 0) {
    if (inet_aton(group, &ip_addr) == 0) {
      close(multicast.sock);
      multicast.sock = -1;
      return multicast;
    }
    multicast.is_mcast = ((((int32_t) ip_addr.s_addr) & MCAST_ADDRESS_MASK)
        == MCAST_ADDRESS_MASK);
  }
  else {
    // Assume unicast receive
    multicast.is_mcast = false;
  }
  ...
  for (ii = 0; ii < devs.ifc_len / sizeof(struct ifreq); ii++) {
    ...
    if (strcmp(iface, devs.ifc_req[ii].ifr_name) == 0) {
      ...
      if (!(dev.ifr_flags & IFF_UP) || (dev.ifr_flags & IFF_LOOPBACK)
          || !(multicast.is_mcast || (dev.ifr_flags & IFF_MULTICAST))) {
        close(multicast.sock);
        multicast.sock = -1;
        free(devs.ifc_buf);
        return multicast;
      ...
      if (multicast.is_mcast) {
        memset(&mreqn, 0, sizeof(mreqn));
        mreqn.imr_multiaddr = ip_addr;
        ...
        multicast.addr.sin_addr.s_addr = INADDR_ANY;
      }
      ...
      if (is_receiver) {
        ...
        if (multicast.is_mcast) {
          ...
          if (setsockopt(multicast.sock, IPPROTO_IP, IP_ADD_MEMBERSHIP, &mreqn,
        ...
        multicast.addr.sin_addr = ip_addr;
      }
      free(devs.ifc_buf);
      return multicast;
    }
...
static inline multicast_t multicast_client(const char* iface,
    const char* group, int port, int size = DEFAULT_SO_RCVBUF) {
  multicast_t client = multicast_open_(iface, group, port, true);
  if (client.sock != -1) {
    if (!client.is_mcast && strcmp(group, "") != 0) {
      // If we get here, we've failed
      close(client.sock);
      client.sock = -1;
      return client;
    ...
    if (size >= 0) {
      if (setsockopt(client.sock, SOL_SOCKET, SO_RCVBUF, &size, sizeof(int))
          != 0) {
        close(client.sock);
        client.sock = -1;
      }
 ...
 */
static inline ssize_t multicast_receive(multicast_t &client, void* buffer,
    size_t bytes, float timeout_in_seconds) {
...
static inline multicast_t multicast_server(const char* iface,
    const char* group, int port, uint8_t ttl, int size = DEFAULT_SO_SNDBUF) {
  if (strcmp(group,"")==0){
    return multicast_none();
  }
  multicast_t server = multicast_open_(iface, group, port, false);
  if (server.sock != -1) {
    if (server.is_mcast) {
      setsockopt(server.sock, IPPROTO_IP, IP_MULTICAST_TTL, &ttl, sizeof(ttl));
    }
    if (size >= 0) {
      if (setsockopt(server.sock, SOL_SOCKET, SO_SNDBUF, &size, sizeof(int))
          != 0) {
        close(server.sock);
        server.sock = -1;
      }
...
static inline ssize_t multicast_transmit(multicast_t &server,
  ...
  ssize_t sent = sendto(server.sock, buffer, bytes, 0, (struct sockaddr*) &server.addr,
      sizeof(server.addr));
//  an exception probably should be thrown here
//  if (sent < 0) {
//    throw VRTException("Could not send data to host=%s port=%d device=%s", host.c_str(), port, device.c_str());
//  }
  return sent;
}
