...
public class PacketGen extends Main {
  ...
  private static final class FileDetails {
    ...
    FileDetails (Node node) {
      this.classify = evalXPath("./CLASSIFICATION", node, "");
  ...
  protected void main () throws Exception {
    ...
    Map<String,PacketType>  packetTypes = new LinkedHashMap<String,PacketType>(16);
    Map<String,FileDetails> fileDetails = new LinkedHashMap<String,FileDetails>(16);
    ArrayList<PacketType>   pktFactList = new ArrayList<PacketType>(4);
    ...
    LinkedHashMap<String,ArrayList<PacketType>>
                            typesBySrc  = new LinkedHashMap<String,ArrayList<PacketType>>(64);
    ...
    for (PacketType p : packetTypes.values()) {
      if (!typesBySrc.containsKey(p.srcFileName)) {
        typesBySrc.put(p.srcFileName, new ArrayList<PacketType>(64));
      }
    ...
    for (String f : typesBySrc.keySet()) {
      ...
      if (dirHTML != null) {
        printHeaderHTML(docHTML, details.classify, details.title, details.author, details.descr);
      ...
      if (dirHTML != null) {
        printFooterHTML(docHTML, details.classify);
        docHTML.close();
      }
    }
  ...
  private static PrintStream new_PrintStream (File f) throws IOException {
    ...
    return new PrintStream(new BufferedOutputStream(new FileOutputStream(f), 65536));
  }

  /** Prints an HTML header. */
  static void printHeaderHTML (PrintStream out, String classify, String title) {
    printHeaderHTML(out, classify, title, null, null);
  }

  /** Prints an HTML header with title block. */
  static void printHeaderHTML (PrintStream out, String classify, String title, String author, String description) {
    out.println("<html>");
    out.println("  <head>");
    ...
    if ((classify != null) && (classify.length() > 0)) {
      out.println("    <center><b>" + toHtml(classify) + "</b></center>");
      out.println("    <hr>");
    }
    if ((title != null) && (title.length() > 0)) {
  ...
  static void printFooterHTML (PrintStream out, String classify) {
    if ((classify != null) && (classify.length() > 0)) {
      out.println("    <hr>");
      out.println("    <center><b>" + toHtml(classify) + "</b></center>");
    }
  ...
  static void printIndexHTML (PrintStream out, String classify, String title, String author,
