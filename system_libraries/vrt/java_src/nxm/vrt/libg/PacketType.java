...
class PacketType extends Record {
  ...
  final List<Enumeration>        enums     = new ArrayList<Enumeration>(16);
  final Map<String, Enumeration> enumMap   = new LinkedHashMap<String, Enumeration>(16);
  final Map<String, Record>      recordMap = new LinkedHashMap<String, Record>(16);
  final Map<String, XPath>       xpathMap  = new LinkedHashMap<String, XPath>(16);
   ...
   */
  PacketType (Node node, Map<String, PacketType> packetTypes, String srcFileName) {
    super(node);
  ...
  private String getParentClassJava (PacketGen outParams) {
    ...
    if (p instanceof PacketType) {
      return outParams.javaPackage + "." + ((Record)p).name;
    }
  ...
  public void writeHTML (PacketGen outParams, PrintStream html, PrintStream index, String fileName) {
    printIndexHTML(index, classify, sect, author, descr, fileName);
    printHeaderHTML(html, classify, sect);
  ...
  public void writeCPP (PacketGen outParams, PrintStream hdr, PrintStream cpp) {
    ...
    hdr.println("    public:    virtual Value*    getField (int32_t id) const __attribute__((warn_unused_result));");
    ...
    cpp.println("}");
    cpp.println("");
    cpp.println(name + "::" + name + " (const vector<char> &_bbuf, bool _readOnly) :");
    cpp.println("  "+superClass+"(_bbuf, _readOnly)");
    cpp.println("{");
    cpp.println(" // done");
    cpp.println("}");
    cpp.println("");
      ...
      if (f.methodName != null) {
        ...
        if (t.isArrayType()) {
          ...
          if (typ.isStringType()) {
            ...
            cpp.println("      str << \"[\" << get" + singleName + "(i).toString() << \"]\";");
          } if (typ instanceof Record) {
            cpp.println("      str << \"[\" << get" + singleName + "(i).toString() << \"]\";");
          }
          else if (typ.isObjectType()) { // explicit .toString() for clang
            cpp.println("      str << get" + singleName + "(i).toString();");
          }
          else {
            cpp.println("      str << get" + singleName + "(i);");
        ...
        else if (t.isUTF8Type() || t.isUTF8TypeStr()) {
          cpp.println("  str << \" " + f.name + "=\'\" << Utilities::fromUTF8(get" + f.methodName + "()) << \"\'\";");
        }
        else if (t.isStringType()) {
          cpp.println("  str << \" " + f.name + "=\'\" << get" + f.methodName + "() << \"\'\";");
        }
        else if (t.isMetadataType()) {
          cpp.println("  str << \" " + f.name + "=[\" << get" + f.methodName + "().toString() << \"]\";");
        }
        else if (t instanceof Record) {
          cpp.println("  str << \" " + f.name + "=[\" << get" + f.methodName + "().toString() << \"]\";");
        }
        else if (t.isObjectType()) { // explicit .toString() for clang
          cpp.println("  str << \" " + f.name + "=[\" << get" + f.methodName + "().toString() << \"]\";");
        }
        else {
          cpp.println("  str << \" " + f.name + "=\" << get" + f.methodName + "();");
  ...
  public void writeJava (PacketGen outParams, PrintStream out) {
    ...
    out.println("  }");
    out.println("  ");
    out.println("  /** <b>Not for General Use:</b> Creates a new instance from an existing VRTPacket,");
    out.println("   *  see {@link nxm.vrt.lib.BasicVRTPacket#BasicVRTPacket(VRTPacket,boolean)} for details.");
    out.println("   *  @param p    The existing packet.");
    out.println("   *  @param copy Must the data be copied?");
    out.println("   *  @throws NullPointerException If <tt>p</tt> is null.");
