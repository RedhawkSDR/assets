...
public class Main {
   ...
   */
  protected Main (String[] version, String[] usage, String[] supportedOptions,
                  int minArgs, int maxArgs, String[] args) {
    Map<String,String> opts   = new LinkedHashMap<String,String>(8);
  ...
  private static void parseCmdLine (Map<String,String> opts, List<String> params,
    ...
    if (helpVer) {
      ...
      int len = supportedOptions.length;
      supportedOptions = Arrays.copyOf(supportedOptions, len+1);
      supportedOptions[len++] = "--dumpcshscript";
    }
    ...
    for (String opt : supportedOptions) {
      ...
      if (opt.equals(key)) {
        if (!hasValue) { // state value
          try {
            Boolean v = Utilities.toBoolean(val);
            if ((v == null) || v) {
              opts.put(key, ""); // present or true
            }
            return index;
          }
  ...
  private <T extends Enum<T>> T getEnumOption (String key, T def, Class<T> clazz) {
    String val = options.get(key);
    try {
      T value = Utilities.toEnum(clazz, val);
      return (value == null)? def : value;
    }
    catch (IllegalArgumentException e) {
      throw new InvalidCommandLineException("Value of "+key+"= must be one of specified values, but given "+val);
    }
  }
   ...
   *  @return The content for the script with embeded line breaks.
   */
  ...
  /** Runs a new instance.
   *  @param <T>              The specific subclass being used.
   ...
   *  @throws InvalidCommandLineException If there is a syntax error on the command line.
   */
  @SuppressWarnings({"UseOfSystemOutOrSystemErr","CallToPrintStackTrace","CallToThreadDumpStack"})
