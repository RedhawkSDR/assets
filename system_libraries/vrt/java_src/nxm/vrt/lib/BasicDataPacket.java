...
public class BasicDataPacket extends BasicVRTPacket implements DataPacket {
  ...
  /** <b>Not for General Use:</b> Creates a new instance from an existing VRTPacket,
   *  see {@link BasicVRTPacket#BasicVRTPacket(VRTPacket,boolean)} for details.
   *  @param p    The existing packet.
   *  @param copy Must the data be copied?
   *  @throws NullPointerException If <tt>p</tt> is null.
   *  @throws IllegalArgumentException If the packet given does not match this type.
   */
   ...
   *  @param copy             Is a copy be required?
  ...
  public void setAssocPacketCount (Byte v) {
    ...
    if (v == null) {
      ...
      if (tbuf == null) bbuf[boff+getPacketLength()-1] = 0;
      else              tbuf[toff+3]                   = 0;
      ...
      byte val = (byte)(0x80 | v); // include indicator bit
      if (tbuf == null) bbuf[boff+getPacketLength()-1] = val;
      else              tbuf[toff+3]                   = val;
    }
  }

  /** If the trailer is empty, delete it. */
  protected final void dropTrailerIfEmpty () {
    byte[] buf = getTBuf();
    int    off = getTOff();

    if ((buf[off] == 0) && ((buf[off+1] & 0xF0) == 0) && ((buf[off+3] & 0x80) == 0)) {
      shiftTrailer(-4);
    }
  }
  ...
  @Override public final int  getScalarDataLength ()                                { return getDataLength(getPayloadFormat(), true ); }
  @Override public final int  getScalarDataLength (PayloadFormat pf)                { return getDataLength(pf,                 true ); }
  @Override public final int  getDataLength ()                                      { return getDataLength(getPayloadFormat(), false); }
  @Override public final int  getDataLength (PayloadFormat pf)                      { return getDataLength(pf,                 false); }
  @Override public final void setScalarDataLength (int len)                         { setDataLength(getPayloadFormat(), len, true ); }
  @Override public final void setScalarDataLength (PayloadFormat pf, int len)       { setDataLength(pf,                 len, true ); }
  @Override public final void setDataLength (int len)                               { setDataLength(getPayloadFormat(), len, false); }
  @Override public final void setDataLength (PayloadFormat pf, int len)             { setDataLength(pf,                 len, false); }
  ...
  public void setDataFloat (PayloadFormat pf, float[] array) {
    setScalarDataLength(pf, array.length);
   ...
   *  @param pf     The payload format (not null).
   *  @param scalar Assume a scalar-only computation?
   *  @return The number of elements.
   */
  protected int getDataLength (PayloadFormat pf, boolean scalar) {
    int     bitsPerSample = pf.getItemPackingFieldSize();
    int     complexMult   = (scalar || !pf.isComplex())? 1 : 2;
    boolean powerOfTwo    = (bitsPerSample == 64) || (bitsPerSample == 32) ||
  ...
  public int getLostSamples (TimeStamp expected, double sampleRate) {
    ...
    long timeDeltaPS  = ts.getPicoSeconds() - expected.getPicoSeconds();
  ...
  // with -O3 and no debug lines in this function).
   ...
   *  @param scalar Assume a scalar-only computation?
   */
  protected void setDataLength (PayloadFormat pf, int length, boolean scalar) {
    int     bitsPerSample = pf.getItemPackingFieldSize();
    int     samples       = (scalar || !pf.isComplex())? length : length*2;
    boolean powerOfTwo    = (bitsPerSample == 64) || (bitsPerSample == 32) ||
                            (bitsPerSample == 16) || (bitsPerSample ==  8) ||
                            (bitsPerSample ==  4) || (bitsPerSample ==  2) ||
                            (bitsPerSample ==  1); // ordered to match common vals first
