...
public class BasicVRTPacket implements VRTPacket, HasFields {
  ...
  /** <b>Not for General Use:</b> Creates a new instance from an existing VRTPacket. This is used
   *  internal to the various packet factories (see {@link PacketFactory}) to "convert" between
   *  generic versions of a packet and specific forms with a minimum of data copies. Nearly all
   *  instances of {@link VRTPacket} have a constructor that matches this. <br>
   *  <br>
   *  This constructor is not strictly "internal use only"; however, its use should be restricted
   *  to calls internal to a {@link PacketFactory} and/or calls to <tt>super(p,copy)</tt> from
   *  the <tt>MyPacket(VRTPacket,boolean)</tt> constructor in a sub-class of {@link BasicVRTPacket}.
   ...
   *  @param copy             Must the data be copied?
  ...
  BasicVRTPacket (byte[] bbuf, int boff, int blen,
    ...
    if ((pbuf == null) != (tbuf == null)) {
      throw new AssertionError("Can not have only one of pbuf and tbuf set to null.");
    }
  }
   ...
   *  @param type     The packet type.
   *  @param streamID The stream ID.
   *  @param classOUI The OUI of the class ID.
   *  @param classICC The ICC of the class ID.
   *  @param classPCC The PCC of the class ID.
   */
  @SuppressWarnings("OverridableMethodCallInConstructor")
  ...
  /** Creates a new instance that can be written to. This packet will be in "standard form" with
   *  the type, stream ID and classID given. The time will be set to 00-00-1970T00:00:00.000 UTC. <br>
   *  <br>
   *  Note that the length of the payload buffer is NOT specified. It is assumed that the payload
   *  length is initially zero, but may grow as content is added to it.
   *  @param type     The packet type.
   *  @param streamID The stream ID.
   *  @param classOUI The OUI of the class ID.
   *  @param classICC The ICC of the class ID.
   *  @param classPCC The PCC of the class ID.
   *  @param pbuf     The payload buffer.
   *  @param poff     The offset into the payload buffer.
   */
  @SuppressWarnings("OverridableMethodCallInConstructor")
  public BasicVRTPacket (PacketType type, int streamID, int classOUI, short classICC, short classPCC,
                         byte[] pbuf, int poff) {
  ...
  public long getStreamCode () {
    switch (bbuf[boff] & 0xF0) {
      case 0x00: return 0x0000000000000000L; // no stream identifier
      case 0x20: return 0x2000000000000000L; // no stream identifier
      default:   return unpackLong(bbuf, boff) & 0xF0000000FFFFFFFFL;
    }
  }
   ...
   *  @return The default header for the packet.
   */
   ...
   *  @return The default payload for the packet (usually {@link #EMPTY}).
   */
   ...
   *  @return The default trailer for the packet (usually {@link #EMPTY}). The only options for
   *          this include {@link #EMPTY} or an array of length 4.
   */
  ...
  public final String toString () {
    try {
      ...
      return getClass().getName()+": <Invalid VRTPacket: "+e+">";
    }
  }
  ...
  public final boolean trailerEquals (VRTPacket p2, boolean exact) {
    ...
    if (exact) {
      ...
      for (int e=20,i=8; e <= 31; e++,i++) {
        ...
        if (!Utilities.equal(bit1, bit2)) return false; // state/event bits don't match
      }
      return true;
    }
  }
  ...
  private int getOffset (int field) {
    int a0  = (bbuf[ boff ] | 0x02) & 0xFF;
    int b0  = (bbuf[boff+1]       ) & 0xFF;
    int a   = a0 & (field >> 8);
    int b   = b0 & (field     );
  ...
  final void shiftTrailer (int bytes) {
    int oldLen = getPacketLength();
    int newLen = oldLen + bytes;

    if (bytes < 0) {
      bbuf[boff] &= ~TRAILER_BIT;
    }
    else if (tbuf != null) {
      memset(tbuf, toff, 0, 4); // always fixed at 4
      bbuf[boff] |= TRAILER_BIT;
    }
    else {
      memset(bbuf, boff+oldLen-1, 0, 4); // <-- may throw exception if insufficient size
      bbuf[boff] |= TRAILER_BIT;
    }
   ...
   */
  public static int getPacketLength (byte[] buf, int off) {
    return ((0xFF & buf[off+2]) << 10) | ((0xFF & buf[off+3]) << 2);
  }
   ...
   *  {@link VRLFrame#readFrameFrom(InputStream)}). <br>
   *  <br>
   *  See {@link BasicVRLFrame#readFrameFrom(InputStream,byte[],int)} for a version that supports
   *  both VRT packets and VRL frames.
   ...
   *  @return The offset into the payload buffer.
   */
   ...
   *  @return The pointer to the trailer buffer.
   */
   ...
   *  @return The offset into the trailer buffer.
   */
