...
public final class TimeStamp implements Comparable<TimeStamp>, HasFields {
   ...
   */
  public static final double ONE_PICOSECOND = 1e-12;
   ...
   *  </pre>
   */
  public static final double HALF_PICOSECOND = 0.500e-12;
   ...
   *  @return The current time stamp per the local system time.
   */
    ...
    switch (tsiMode) {
      case None:
        switch (tsfMode) {
          ...
          case RealTime:         return toPicosecondString(tsf) + " (RealTime)";
          case FreeRunningCount: return Long.toString(tsf)      + " (FreeRunningCount)";
        }
        break;
      case UTC:
        switch (tsfMode) {
          case None:             return ls.toStringUTC(tsi, -1L)                            + " (UTC)";
          case SampleCount:      return ls.toStringUTC(tsi, -1L) + "," + Long.toString(tsf) + " (UTC,SampleCount)";
          case RealTime:         return ls.toStringUTC(tsi, tsf)                            + " (UTC)";
          case FreeRunningCount: return ls.toStringUTC(tsi, -1L) + "," + Long.toString(tsf) + " (UTC,FreeRunningCount)";
        }
        break;
      case GPS:
        switch (tsfMode) {
          case None:             return LeapSeconds.toStringGPS(tsi, -1L)                            + " (GPS)";
          case SampleCount:      return LeapSeconds.toStringGPS(tsi, -1L) + "," + Long.toString(tsf) + " (GPS,SampleCount)";
          case RealTime:         return LeapSeconds.toStringGPS(tsi, tsf)                            + " (GPS)";
          case FreeRunningCount: return LeapSeconds.toStringGPS(tsi, -1L) + "," + Long.toString(tsf) + " (GPS,FreeRunningCount)";
        }
        break;
      case Other:
        switch (tsfMode) {
          ...
          case RealTime:         return Long.toString(tsi) + "," + toPicosecondString(tsf) + " (Other,RealTime)";
          case FreeRunningCount: return Long.toString(tsi) + "," + Long.toString(tsf)      + " (Other,FreeRunningCount)";
        }
        break;
    }
    throw new AssertionError("Invalid mode combination -- this should be impossible");
  }

  /** A textural representation of the time stamp in UTC. The output from this
   *  is identical to the time stamp produced by <tt>ts.toUTC().toString()</tt>
   *  except that the " (UTC)" suffix is omitted and fractional modes
   *  <tt>SampleCount</tt> and <tt>FreeRunningCount</tt> are not supported.
   *  @return A textural representation of the time stamp.
   */
  public String toStringUTC () {
    ...
    long sec  = getUTCSeconds();
    long psec = (tsfMode == FractionalMode.None)? -1L : getPicoSeconds();
    return ls.toStringUTC(sec, psec);
  }

  /** A textural representation of the time stamp in GPS. The output from this
   *  is identical to the time stamp produced by <tt>ts.toGPS().toString()</tt>
   *  except that the " (GPS)" suffix is omitted and fractional modes
   *  <tt>SampleCount</tt> and <tt>FreeRunningCount</tt> are not supported.
   *  @return A textural representation of the time stamp.
   */
  public String toStringGPS () {
    ...
    // want to avoid creating an extra TimeStamp object.
    long sec  = getGPSSeconds();
    long psec = (tsfMode == FractionalMode.None)? -1L : getPicoSeconds();
    return LeapSeconds.toStringGPS(sec, psec);
  }

  /** Gets the epoch used.
   *  @return {@link #GPS_EPOCH} or {@link #UTC_EPOCH} or null if neither apply.
   *          This will return null any time {@link FractionalMode#SampleCount}
   *          or {@link FractionalMode#FreeRunningCount} is in use.
   *  @see #getIntegerMode()
   ...
   *  @return The mode for the integer portion of the time stamp.
   ...
   *  @return The mode for the fractional portion of the time stamp.
  ...
  public final double getFractionalSeconds () {
    return getPicoSeconds() * ONE_PICOSECOND;
  }
  ...
  private static TimeStamp _forTimeNORAD (int seconds, long picoseconds, boolean leapCounted,
    ...
    if (leapCounted) {
      ...
      return new TimeStamp(IntegerMode.UTC, FractionalMode.RealTime, sec, picoseconds, ls);
    }
    else {
